...
use rime_lib::{CatalogType, Config, SnapshotMonitor, TableAnalyser};
...
async fn init_config(config_path: &PathBuf, force: bool) -> anyhow::Result<()> {
    ...
    config.catalogs.insert(
        "glue-example".to_string(),
        rime_lib::CatalogConfig::Glue {
    ...
    config.catalogs.insert(
        "rest-example".to_string(),
        rime_lib::CatalogConfig::Rest {
...
async fn load_config(config_path: &PathBuf) -> anyhow::Result<Config> {
    if !config_path.exists() {
        anyhow::bail!(
            "Configuration file not found: {}. Run 'rime init' first.",
            config_path.display()
        );
    }
...
async fn get_analyser(
    config: &Config,
    catalog_name: &Option<String>,
) -> anyhow::Result<TableAnalyser> {
...
async fn list_namespaces(
    ...
    match analyser.list_namespaces().await {
        ...
        Err(e) => {
            error!(
                "Failed to list namespaces from catalog '{}': {}",
                catalog_to_use, e
            );
...
fn print_table_analysis(result: &rime_lib::AnalysisResult) {
    ...
    println!(
        "  Total Size: {:.2} GB",
        result.metrics.total_size_bytes as f64 / (1024.0 * 1024.0 * 1024.0)
    );
    ...
    println!(
        "  Needs Compaction: {}",
        result.maintenance_info.needs_compaction
    );
    println!(
        "  Small Files: {}",
        result.maintenance_info.small_files_count
    );
    println!(
        "  Estimated Orphaned Files: {}",
        result.maintenance_info.orphaned_files_estimated
    );
    ...
    println!(
        ...
        result.performance_info.avg_file_size_mb
    );
    println!(
        "  Scan Efficiency Score: {:.1}",
        result.performance_info.scan_efficiency_score
    );
    println!(
        "  Partition Pruning Effectiveness: {:.1}",
        result.performance_info.partition_pruning_effectiveness
    );
}
...
async fn show_maintenance(
    ...
    match analyser.analyse_table(&namespace_vec, table_name).await {
        Ok(result) => {
            ...
            println!(
                "Estimated Orphaned Files: {}",
                info.orphaned_files_estimated
            );

            if let Some(last_compaction) = &info.last_compaction {
                println!(
                    "Last Compaction: {}",
                    last_compaction.format("%Y-%m-%d %H:%M:%S UTC")
                );
            } else {
...
async fn show_performance(
    ...
    match analyser.analyse_table(&namespace_vec, table_name).await {
        Ok(result) => {
            ...
            println!(
                "Scan Efficiency Score: {:.1}/1.0",
                perf.scan_efficiency_score
            );
            println!(
                "Partition Pruning Effectiveness: {:.1}/1.0",
                perf.partition_pruning_effectiveness
            );
    ...
    if let Some(namespace) = namespace_filter {
        ...
        for table_spec in tables {
            if let Some((namespace, table_name)) = parse_table_spec(&table_spec) {
                ...
                error!(
                    "Invalid table specification: {}. Use format 'namespace.table'",
                    table_spec
                );
